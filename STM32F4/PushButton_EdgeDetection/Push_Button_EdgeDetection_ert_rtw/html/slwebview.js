...
function initialize(evt)
{
    ...
    svgState.canvasBackground = svgState.svgDoc.getElementById('canvas.background');
...
function getSvgState(id)
{
    ...
    if ((typeof top._slwebview === "undefined")
        && (navigator.appVersion.toLowerCase().indexOf("webkit") != -1) 
        && (window.location.origin === "file://")) {
        
        var msg1 = 'Simulink Web View cannot currently be viewed in Google Chrome because of local file security restrictions.  To allow local file access, try the following:\n';
        var msg2 = 'Microsoft Windows\n';
        var msg3 = '   1. Click on the Windows start button and search for "Chrome".\n';
        var msg4 = '   2. Right-click and drag "Google Chrome" search result to an open area.  Select "Create shortcuts here".\n';
        var msg5 = '   3. Right-click the newly created "Google Chrome" shortcut and select properties.\n';
        var msg6 = '   4. Click the "Shortcut" tab.\n';
        var msg7 = '   5. In the target edit box, append "--allow-file-access-from-files", with no quotes.\n';
        var msg8 = '   6. Click OK to dismiss property dialog\n';
        var msg9 = '   7. Verify Google Chrome is no longer running.\n';
        var msg10 = '   8. Double-click on the newly created shortcut to start Google Chrome.\n';
        var msg11 = 'Macintosh\n';
        var msg12 = '   1. Run Terminal.app.\n';
        var msg13 = '   2. Type "open /Applications Google\ Chrome.app --allow-file-access-from-files", with no quotes.\n';
        var msg14 = 'Linux\n'
        var msg15 = '   1. Start terminal.\n';
        var msg16 = '   2. Type "./chromium-browser --allow-file-access-from-files", with no quotes.\n';
...
function svgStateObj()
{
    ...
    this.canvasBackground = null;       // background
...
function exposeAll()
{
    ...
    // recalculate dim size after window is resize if necessary.
    if (wHeight < 0) {
        dim = getBrowserInnerWindowDimensions();
        wWidth  = dim[0];
        wHeight = dim[1];
        if (wHeight < 0)
            wHeight = 100;
    ...
    svgState.canvasBackground.setAttribute('fill','none');
...
function zoomRelative(zoomFactor, evt)
{
    ...
    if (currCanvas && !svgState.isShowingAllCanvas) {
        ...
        // Set the size via the viewbox.
        setViewBox(currCanvas, vbCurrArray);
    }
}
...
function doMousePan(evt)
{
    ...
    if (currCanvas && !svgState.isShowingAllCanvas)
    {
        ...
        setViewBox(currCanvas, newXo + ' ' + newYo + ' '
...
function changeSys(evt)
{
    ...
    if (typeof(top) != "undefined" && typeof(top.rtwChangeSysCallback) == "function") {
        var id = getCurrentTargetId(evt);
        top.HiliteCodeStatus = top.rtwChangeSysCallback(id);
        var svgState = getSvgState();
        top.HiliteBlock = id;
        top.HiliteCanvas = svgState.currSys;
        gHiliteBlockAfterChangeSys = true;
    }
...
function changeSysByName(newSys,addToPrevStack)
{
    ...
    if (newSysCanvas) {
        dismissPropertyTables(newSysCanvas);
        doChangeSys(newSys,newSysCanvas,addToPrevStack);
    } else {
        loadAndChangeSys(newSys,addToPrevStack);
    }
    // highlight block after changeSys in RTW
    if (gHiliteBlockAfterChangeSys) {
        rtwHiliteBlockAfterChangeSys();
    }
}
...
// hilite block after changeSys
var gHiliteBlockAfterChangeSys = false;
var gLastHiliteSuccessful = null;
...
function loadAndChangeSys(newSys,addToPrevStack)
{
    if (newSys) {
        ...
        try {
            ...
            if (typeof gSLFcnId2SubsysId === "undefined") {
                return; 
            } else {
                try { 
                    /* replace SLFcn's SF obj id with its Simulink system sid */
                    newSys = gSLFcnId2SubsysId[newSys].subsystem;
                    filename = gIDMap[newSys].target;
                    newSysCanvas = findCanvasById(newSys);
                    if (newSysCanvas) {
                    	// the SLFcn has been loaded
                    	dismissPropertyTables(newSysCanvas);        
                        doChangeSys(newSys,newSysCanvas,addToPrevStack);                    	
                    	return;
                    }
        ...
        if (typeof svgState.svgWindow.getURL == "function") {
            ...
            if (location.protocol == "file:") {
                // IE9, XMLHttpRequest fails when accessing local file
                try { 
...
function changeSysByLoadingASV(data)
{
    ...
    if (gHiliteBlockAfterChangeSys) {
        rtwHiliteBlockAfterChangeSys();
        gHiliteBlockAfterChangeSys = false; // clear flag after callback
    }
}
...
{
    var svgState = getSvgState(),
        docFrag = gXMLHTTPRequest.responseXML.documentElement,
        newSysCanvas,
        xmlDoc;
    
    if (docFrag) {
        newSysCanvas = svgState.svgDoc.importNode(docFrag,true);
    } else {
        xmlDoc = new ActiveXObject("Microsoft.XMLDOM");
        xmlDoc.async=false; 
        xmlDoc.loadXML(gXMLHTTPRequest.responseText);
        docFrag = xmlDoc.documentElement;
        newSysCanvas = importNode(svgState.svgDoc, docFrag);
    }
...
function updateBackground(sysCanvas)
{
    if (sysCanvas) {
        var svgState = getSvgState();
        var background = findDescendentById(sysCanvas,"background",2);
        if (background) {
            svgState.canvasBackground.setAttributeNS(null,"fill",background.getAttributeNS(null,"fill"));
        }
    }
}
...
function updateTitle(thisObj)
{
    ...
    if (thisObj)
    {
        var thisTitle = findDescendentById(thisObj, 'title.' + thisID);
        if (thisTitle)
        {
...
/**
* Function to Highlight a block for code2model link in RTW codegen report
*/
function hiliteBlockForRTWReport(aBlock, aParentSID)
{
    var addToPrevStack = 1;
    gLastHiliteSuccessful = null;
    if (aBlock.indexOf('-') !== -1) {
    ...
    if (typeof aParentSID === "undefined")
        aParentSID = aBlock;
    // changeSys and highlight block
    if (typeof(top) != "undefined") {
        var filename;
        // test if parent system has canvas
        try {
            filename = gIDMap[aParentSID].target;
        } catch(e) {
            if (typeof gSLFcnId2SubsysId === "undefined") {
                return false;
            } else {
                try {
                    /* replace SLFcn's SF obj id with its Simulink system sid */
                    aParentSID = gSLFcnId2SubsysId[aParentSID].subsystem;
                    filename = gIDMap[aParentSID].target;
                } catch(e) {
                    return false;
                }
        ...
        top.HiliteBlock = aBlock;
        top.HiliteCanvas = aParentSID;
        gHiliteBlockAfterChangeSys = true;
        changeSysByName(aParentSID, addToPrevStack);
    }
    return gLastHiliteSuccessful;
}

/**
 * Highlight block in current canvas after changeSys.
 * Remove highlight for last block
 */
function rtwHiliteBlockAfterChangeSys() {
    if (typeof(top) != "undefined") {
        var svgState = getSvgState();
        var domObj = findDescendentById(svgState.currSysCanvas , top.HiliteBlock)
        if (domObj != false)
            updateTitle(domObj);
        if(domObj != false){
            highlight2(domObj);
            // unhighlight the last highlighted block
            rtwUnHiliteLastBlock(); 
            top.LastHiliteBlock = top.HiliteBlock;
            top.LastHiliteCanvas = top.HiliteCanvas;
            top.HiliteBlock = '';
            top.HiliteCanvas = '';
            if (gLastHiliteSuccessful === null) {
                gLastHiliteSuccessful = true;
            }
        } else if (svgState.currSys === top.HiliteCanvas){
            rtwUnHiliteLastBlock();
            top.LastHiliteBlock = top.HiliteBlock;
            top.LasthiliteCanvas = top.HiliteCanvas;
            top.HiliteBlock = '';
            top.HiliteCanvas = '';
            if (gLastHiliteSuccessful === null) {
                gLastHiliteSuccessful = false;
            }
        }
    }
}

function rtwUnHiliteLastBlock() {
    if (top.HiliteBlock === top.LastHiliteBlock && top.HiliteCanvas === top.LastHiliteCanvas) {            
        return;
    }
    if (typeof(top) != "undefined") {
        // unhighlight the last highlighted block        
        if(top.LastHiliteBlock != null && top.LastHiliteBlock != ""){
            var domObj = findDescendentById( findCanvasById(top.LastHiliteCanvas) , top.LastHiliteBlock ); 
            if( domObj != false){
                unhighlight2(domObj); // domObj.setAttributeNS(null,"filter","none");                               
                top.LastHiliteBlock = '';
                top.LastHiliteCanvas = '';
            }
        }
    }
}
...
function getPropGroup(evt, id)
{
    ...
    var svgState = getSvgState();
    if (!prop) {
        // will be a child if already shown (for prop sheets, not help button)
        prop = findChildByIdReverse(svgState.currSysCanvas, 'prop.' + id);
    }
    ...
    if (prop) {
        if (isHelp) {
            ...
            moveToTopOfCanvas(prop,svgState.currSysCanvas);
        }
    }
...
function moveToTopOfCanvas(obj,canvas)
{
    var svgState = getSvgState();
...
function displayByElement(targetElement, evt)
{
    if (targetElement) {
        ...
        var firstElement = targetElement.firstChild;
        if (firstElement.nodeType === firstElement.TEXT_NODE) {
            firstElement = firstElement.nextSibling;
        }
        
        if (firstElement.nodeType === firstElement.ELEMENT_NODE) {
            firstElement.setAttributeNS(null, 'filter', 'none');
        }
...
function highlight(evt,isForce)
{
    ...
    if (!tCursor) {
        ...
        if (((isForce == 1) || gIDMap[linkid]) && (linkid != svgState.currSys)) {
            tCursor = "pointer";
        } else {
            tCursor = "default";
        }
    ...
    if (tCursor == "pointer") {
        if (navigator.appVersion.toLowerCase().indexOf("webkit") == -1) {
            // Allow highlighting on non-webkit browser, because webkit turns highlighted element invisible
...
/** 
 * RTW highlight functions for webkit without filter support
 */
function highlight2(highlightSysElement)
{
    setElementColor(highlightSysElement, true);
}

function unhighlight2(highlightSysElement)
{
    setElementColor(highlightSysElement, false);
}

function setElementColor(highlightSysElement, set_operation) {
    var bgColor;
    if (top.HiliteCodeStatus)
        bgColor = "0%,81%,81%";
    else
        bgColor = "91%,82%,32%";
    if (highlightSysElement && highlightSysElement.hasAttribute("highlight")) {
        var mode = highlightSysElement.getAttribute("highlight");
        var groupElements = highlightSysElement.getElementsByTagName("g");
        var pathElements = highlightSysElement.getElementsByTagName("path");
        for (var j=0;j<groupElements.length;++j) {
            if (groupElements.item(j).hasAttribute("id"))
                break;
            var fEC = null;
            if (typeof groupElements.item(j).firstElementChild !== "undefined") {
                fEC = groupElements.item(j).firstElementChild;
            } else {
                for (var k=0;k<groupElements.item(j).childNodes.length;k++) {
                    if (groupElements.item(j).childNodes.item(k).nodeType === 1) {
                        fEC = groupElements.item(j).childNodes.item(k);
                        break;
                    }
                }
            }
            /* handle <g><clipPath><path> */
            if (fEC !== null && fEC.nodeName === "clipPath" && 
                typeof fEC.firstElementChild !== "undefined") {
                fEC = fEC.firstElementChild;
            }
            if (fEC !== null && fEC.nodeName === "path") {
                if (groupElements.item(j).hasAttribute("fill") && 
                    groupElements.item(j).getAttribute("fill") !== "none") {
                    if (set_operation) {
                        updateObjFill(groupElements.item(j),bgColor);
                    } else {
                        restoreObjFill(groupElements.item(j));
                    }
                }
                if (mode !== "fill" && groupElements.item(j).hasAttribute("stroke") && 
                    groupElements.item(j).getAttribute("stroke") !== "none") {
                    if (set_operation) {
                        updateObjStroke(groupElements.item(j),bgColor);
                    } else {
                        restoreObjStroke(groupElements.item(j));
                    }
                }                    
            }
            var imgs = groupElements.item(j).getElementsByTagName("image");
            for (var k=0;k<imgs.length;k++) {
                if (set_operation) {
                    transparentize_img(imgs.item(k));
                } else {
                    restore_opacity(imgs.item(k));
                }
            }
        }
    }
    /* transparentize image element in highlighting */
    function transparentize_img(ele) {
        if(!ele.hasAttribute("orig_opacity")) {
            if (ele.hasAttribute("opacity")) {
                ele.setAttribute("orig_opacity", ele.getAttribute("opacity"));
            } else {
                ele.setAttribute("orig_opacity", "none");
            }
        }
        ele.setAttribute("opacity", "0.2");
    }
    /* restore the opacity attribute */
    function restore_opacity(ele) {
        if (ele.hasAttribute('orig_opacity')) {
            if (ele.getAttribute('orig_opacity') !== '')
                ele.setAttribute('opacity',ele.getAttribute('orig_opacity'));
            else
                ele.removeAttribute('opacity');
            ele.removeAttribute('orig_opacity');
        }
    }
}

function updateObjStroke(child, color) {
    if (!child.hasAttribute('orig_stroke')) {
        if (child.hasAttribute('stroke')) 
            child.setAttribute('orig_stroke', child.getAttribute('stroke'));
        else
            child.setAttribute('orig_stroke', 'none');
        child.setAttribute('stroke','rgb('+color+')');
    } else {
        child.setAttribute('stroke','rgb('+color+')');
    }
}

function updateStroke(cElement, color) {
    for (var j=0;j<cElement.length;++j) {
        child = cElement.item(j);
        updateObjStroke(child, color);
    }
}

function restoreObjStroke(child) {
    if (child.hasAttribute('orig_stroke')) {
        if (child.getAttribute('orig_stroke') !== '')
            child.setAttribute('stroke',child.getAttribute('orig_stroke'));
        else
            child.removeAttribute('stroke');
        child.removeAttribute('orig_stroke');
    }
}

function restoreArrayStroke(cElement) {
    for (var j=0;j<cElement.length;++j) {
        child = cElement.item(j);
        restoreObjStroke(child);
    }
}

function updateObjFill(child, color) {
    if (!child.hasAttribute('orig_fill')) {
        if (child.hasAttribute('fill')) 
            child.setAttribute('orig_fill', child.getAttribute('fill'));
        else
            child.setAttribute('orig_fill', '');        
        child.setAttribute('fill','rgb('+color+')'); // do highlight
    } else {
        child.setAttribute('fill','rgb('+color+')'); // do highlight
    }
}

function updateArrayFill(cElement, color) {
    for (var j=0;j<cElement.length;++j) {
        child = cElement.item(j);
        updateObjFill(child,color);
    }
}

function restoreObjFill(child) {
    if (child.hasAttribute('orig_fill')) {
        if (child.getAttribute('orig_fill') !== '')
            child.setAttribute('fill', child.getAttribute('orig_fill'));
        else
            child.removeAttribute('fill');
        child.removeAttribute('orig_fill');
    }
}

function restoreArrayFill(cElement) {
     for (var j=0;j<cElement.length;++j) {
        child = cElement.item(j);
         restoreObjFill(child);
    }
}
...
function openLinkInWebFrame(url)
 {
    ...
    if (gHiliteBlockAfterChangeSys) {
        var line = "0";
        if (top.HiliteBlock.indexOf(top.HiliteCanvas) != -1) {
            line = top.HiliteBlock.substring(top.HiliteBlock.indexOf(top.HiliteCanvas)+top.HiliteCanvas.length+1);
...
/**
 *  Find descendent node for a given DOM node
 */
function findDescendentById(thisNode, id)
{
    if (!thisNode) {
        return false;
    }
...
{
    if (!thisCanvas || thisCanvas.tagName !== "svg") {
        var svgState = getSvgState();
...
function importNode(svgDoc, node) {
    var i, newNode, nAttrib, nChild, childNode;
    switch (node.nodeType) {
    case svgDoc.ELEMENT_NODE:
        if (node.nodeName == "image") {
            // skip image nodes, as they do not load properly in IE
            return newNode;
        }
        newNode = svgDoc.createElementNS(gSVGNS,node.nodeName);
        // Add attribute
        nAttrib = node.attributes.length;
        if (node.attributes && nAttrib > 0) {
            for (i = 0; i < nAttrib; i++) {
        ...
        // Add children
        nChild =  node.childNodes.length
        if (node.childNodes && nChild > 0) {
            for (i = 0; i < nChild; i++) {
                childNode = importNode(svgDoc, node.childNodes[i]);
                if (childNode) {
        ...
        return newNode;
        break;
    case svgDoc.TEXT_NODE:
    case svgDoc.CDATA_SECTION_NODE:
    case svgDoc.COMMENT_NODE:
        newNode = svgDoc.createTextNode(node.nodeValue);
        return newNode;
        break;
    }
};
...
function setViewBox(canvas, viewBox)
{
    var svgState = getSvgState(),
        corner1,
        corner2,
    ...
    try {
        corner1 = canvas.getElementById(canvas.id + "corner1");
        corner2 = canvas.getElementById(canvas.id + "corner2");
    } catch (ex) {
        // old version of firefox does not support getElementById from svg nodes.
        corner1 = svgState.svgDoc.getElementById(canvas.id + "corner1");
        corner2 = svgState.svgDoc.getElementById(canvas.id + "corner2");
    }
        
    if (typeof viewBox === 'string') {
        vb = new Array();
        vb = viewBox.split(' ');
        vb[3] = parseFloat(vb[3]);
        vb[2] = parseFloat(vb[2]);
        vb[1] = parseFloat(vb[1]);
        vb[0] = parseFloat(vb[0]);
    } else {
        vb = viewBox;
    }
    ...
    if (!corner1 || !corner2) {
        corner1 = svgState.svgDoc.createElementNS(gSVGNS, "rect");
        corner2 = svgState.svgDoc.createElementNS(gSVGNS, "rect");
        
        corner1.setAttribute("id", canvas.id + "corner1");
        corner2.setAttribute("id", canvas.id + "corner2");
        
        corner1.setAttribute("width", 100);
        corner1.setAttribute("height", 100);
        corner2.setAttribute("width", 1);
        corner2.setAttribute("height", 1);
    
        corner1.setAttribute("stroke", "none");
        corner1.setAttribute("fill", "none");
        corner2.setAttribute("stroke", "none");
        corner2.setAttribute("fill", "none");

        canvas.appendChild(corner1);
        canvas.appendChild(corner2);
    }

    corner1.setAttribute("x", vb[0] - 5);
    corner1.setAttribute("y", vb[1] - 5);
    corner2.setAttribute("x", vb[0]+vb[2] + 5);
    corner2.setAttribute("y", vb[1]+vb[3] + 5);    
    
    canvas.setAttributeNS(null, 'viewBox', vb.join(' '));
...
{
    var rectNode = thisCanvas.ownerDocument.createElementNS(gSVGNS,'rect');
